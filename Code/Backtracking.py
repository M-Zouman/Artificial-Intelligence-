...
Button(start_input, text='BackTracking',
...
def check1(k):
    ...
    if k <= n-1 and count <= m-1:
        check1_helper(k-1)
    # add all colors except the last color. red=9, blue=14, green=7 --> 9+14=23 --> 23(-2)=21, 23(-3)=20. always (-2) and (-3).
    if ((n-f-2) <= k <= n - f - 3) and count == m:
        check1_helper(k-1)
