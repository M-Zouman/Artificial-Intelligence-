...
start_input.iconbitmap(
    default="D:\Documents\GitHub\Car-Parking-Lot-Problem\Images\parking_sign.ico")
...
input1 = IntVar()
input2 = IntVar()
Entry(start_input, textvariable=input1, width=50,
      borderwidth=5,).grid(row=0, column=4)
Entry(start_input, textvariable=input2, width=50,
      borderwidth=5,).grid(row=1, column=4)
Button(start_input, text='ForwardChecking',
       command=start_input.destroy).grid(row=4, column=4)
...
car_tuple = (red_car_image, blue_car_image, green_car_image,
             yellow_car_image, indigo_car_image, orange_car_image, violet_car_image)

all_parking_spots = ((35, 38), (87, 38), (139, 38), (191, 38), (243, 38), (35, 145), (87, 145), (139, 145), (191, 145), (243, 145),
                     (408, 38), (460, 38), (512, 38), (564, 38), (616, 38), (408,
                                                                             145), (460, 145), (512, 145), (564, 145), (616, 145),
                     (786, 38), (838, 38), (890, 38), (942, 38), (994, 38), (786,
                                                                             147), (838, 147), (890, 147), (942, 147), (994, 147),
                     (35, 385), (87, 385), (139, 385), (191, 385), (243, 385), (35,
                                                                                492), (87, 492), (139, 492), (191, 492), (243, 492),
                     (408, 385), (460, 385), (512, 385), (564, 385), (616, 385), (408,
                                                                                  492), (460, 492), (512, 492), (564, 492), (616, 492),
...
class ForwardCheck:
    ...
    def CarMaker(number_of_zones, number_of_colors):
        ...
        tuple1 = ('red', 'blue', 'green', 'yellow', 'indigo', 'orange',
                  'violet')  # This tuple contains all possible colors
        # cars can have.
        # The list that will have colors based on the user's amount.
        temp_list = []

        i = 0
        # While loop filling list1 from tuple1, with the amount of colors the user inputted.
        ...
        # each zone has 10 spots hence the total number of parking spots is:
        number_parking_spots = number_of_zones * 10
        # number of zones times 10.
        # maximum number of cars for one color.
        max_num = number_parking_spots / 2
    ...
    color_dict = {1: 'red', 2: 'blue', 3: 'green',
                  4: 'yellow', 5: 'indigo', 6: 'orange', 7: 'violet'}
    reversed_color_dict = {'red': 1, 'blue': 2, 'green': 3,
                           'yellow': 4, 'indigo': 5, 'orange': 6, 'violet': 7}
    ...
    t = 1
    listC = ((50, 180), (190, 180), (333, 180), (477, 180),
             (620, 180), (770, 180), (910, 180))
    while t <= m:
        canvas1.create_text(listC[t-1][0], listC[t-1][1], anchor=NE, font=(
            "Arial", 25), fill=color_dict[t], text=number_of_colors_left[t])
        t = t + 1
    canvas1.update()

    # add a color to colors_available dictionary.

    def ColorsAvailableAdd(self, c):
        colors_available = self.colors_available
        colors_available[c] = colors_available[c] + 1

    # subtract a color from colors_available dictionary.
    def ColorsAvailableSub(self, c):
        colors_available = self.colors_available
        colors_available[c] = colors_available[c] - 1

    # counts all the colors from the given domain, into colors_available dictionary.
    def ColorCount(self):
    ...
    # fills all colors each spot can have before starting.
    def SpotsDomain(self):
    ...
    # checks if the constraints are violated, and returns true or false based on violation occurrence.

    def CheckConstraints(self, k, c):
    ...
    def SpotColorListAdd(self, k, c):
        ...
        if k == 0:
            ...
            return
        if 5 <= k <= 9 or 15 <= k <= 19 or 25 <= k <= 29 or 35 <= k <= 39 or 45 <= k <= 49 or 55 <= k <= 59:
            # if spot k= 9,19,29,39,49 or 59 then no need to check any spots.
            if (k+1) % 5 == 0:
                return
            else:
        ...
        # if spot k= 4,14,24,34,44 or 54 then no need to check k+1.
        elif (k+1) % 5 == 0:
    ...
    def IsEmpty(self, k, c):
        ...
        self.UpdateSpotColorList(
            k, clone_spot_color_list, clone_colors_available)
    ...
    def RunForwardChecking(self, k, c):
        ...
        if self.CheckConstraints(k, c):
            # if FALSE then try another color if there is no more colors backtrack.
            if self.BeforeCheck(k, c):
                if self.IsEmpty(k, c):
                    self.CheckAll(k, c)
                else:
                    self.checked1(k, c)
                ...
                canvas.create_image(
                    all_parking_spots[k][0], all_parking_spots[k][1], anchor=NW, image=car_image_dict[spots[k]])
                time.sleep(0.3)
                ...
                self.UpdateSpotColorList(
                    k+1, spot_color_list, colors_available)

                if k+1 < self.n:
                    self.RunForwardChecking(k+1, self.SpotRetriveValue(k+1))
                else:
                    canvas.create_text(340, 270, anchor=NW, font=(
                        "Arial", 60), fill="green", text="SUCCESS")
                    canvas.update()
                    canvas.update()
                    ...
                    canvas.create_text(800, 270, anchor=NW, font=(
                        "Arial", 60), fill="red", text=run_time)
                    self.canvas1.update()
